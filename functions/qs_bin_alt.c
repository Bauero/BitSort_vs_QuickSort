 ...
 */
static inline void qs_bin_alt_seq(elem_t *arr, long left, long right, int bit) {
    if (left >= right) return;
    if (bit < 0) return; /* stop when no more bits */
    elem_t mask = ((elem_t)1 << bit);
    long i = left, j = right;
    while (i <= j) {
        while (i <= right && (arr[i] < mask)) ++i;
        while (j >= left && (arr[j] >= mask)) --j;
    ...
    if (left < j) qs_bin_alt_seq(arr, left, j, bit - 1);
    if (i < right) qs_bin_alt_seq(arr, i, right, bit - 1);
}
 ...
 *       qs_bin_seq.
 */
static inline void qs_bin_alt_par(elem_t *arr, long left, long right, int bit,
    ...
    if ((right - left) > PARALLEL_THRESHOLD) {
        dispatch_group_async(group, queue, ^{
            qs_bin_alt_par(arr, left, j, bit - 1, queue, group);
        });
        dispatch_group_async(group, queue, ^{
            qs_bin_alt_par(arr, i, right, bit - 1, queue, group);
        });
    } else {
        if (left < j) qs_bin_alt_seq(arr, left, j, bit - 1);
        if (i < right) qs_bin_alt_seq(arr, i, right, bit - 1);
...
void qs_bin_alt_parallel(elem_t *arr, long n, int bit) {
    dispatch_queue_t queue = dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0);
    dispatch_group_t group = dispatch_group_create();
    int bits = (int)(sizeof arr[0] * 8);
    if (bit >= bits) bit = bits - 1;
    qs_bin_alt_par(arr, 0, n, bit, queue, group);
    dispatch_group_wait(group, DISPATCH_TIME_FOREVER);
    // dispatch_release(group); // optional on older APIs
}

void qs_bin_alt_sequential(elem_t *arr, long n, int bit){
    int bits = (int)(sizeof arr[0] * 8);
    if (bit >= bits) bit = bits - 1;
    if (bit < 0) return;
    qs_bin_alt_seq(arr, 0, n, bit);
}
